...
int main(u64 arg1, u64 arg2, u64 arg3, u64 arg4, u64 arg5)
{
    ...
    int strace_fd1 = create_trace_buffer(O_RDWR);
    int rdwr_fd1 = create_trace_buffer(O_RDWR);
    u64 strace_buff1[4096];
    int read_buff1[4096];

    start_strace(strace_fd1, FULL_TRACING);
    int read_ret1 = read(rdwr_fd1, read_buff1, 10);
    if (read_ret1 != 0)
    {
    ...
    int strace_ret1 = read_strace(strace_fd1, strace_buff1, 2);
    if (strace_ret1 != 32)
    {
        printf("2.Test case failed\n");
        return -1;
    }
    if (strace_buff1[0] != 24)
    {
        printf("3.Test case failed\n");
        return -1;
    }
    if ((u64 *)(strace_buff1[2]) != (u64 *)&read_buff1)
    {
